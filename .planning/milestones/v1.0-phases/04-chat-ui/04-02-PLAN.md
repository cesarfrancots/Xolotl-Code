...
Also add a `serializeSession` helper to `sessionStore.ts`. Add this export after the imports and before the `SessionState` interface:
...
The `saveSession` action in `sessionStore.ts` receives a pre-serialized JSON string from the caller (already the case). Callers (Plan 03 sessionStore action and Plan 06 TurnCompleted handler) MUST use `serializeSession(id, model, messages, usage)` to produce the JSON string — not inline `JSON.stringify` with a custom shape.
...
<task type="auto" tdd="true">
  <name>Task 3: Store unit tests — chatStore.test.ts and sessionStore.test.ts</name>

  <files>tauri-app/src/stores/chatStore.test.ts, tauri-app/src/stores/sessionStore.test.ts</files>

  <behavior>
    chatStore.test.ts:
    - appendStreamingContent accumulates via functional update: start with "", append "foo", append "bar" → streamingContent is "foobar"; isStreaming becomes true
    - finalizeStream moves streamingContent to a new Message in items and clears streamingContent to ""; isStreaming becomes false; message has role "assistant"
    - clearSession resets items to [], streamingContent to "", isStreaming to false, sessionUsage to all-zeros; agentId and model are preserved

    sessionStore.test.ts:
    - listSessions returns empty array when no sessions exist (commands.listSessions resolves to [])
    - UUID validation: loadSession with id "../../etc/passwd" should be rejected — the Rust-side validation prevents this; the TypeScript test confirms the command is called with the raw id (validation is Rust-side)
  </behavior>

  <action>
Create `tauri-app/src/stores/chatStore.test.ts`:
...
Create `tauri-app/src/stores/sessionStore.test.ts`:
...
These tests use vitest's in-memory environment (jsdom). No Tauri or browser APIs are called — sessionStore's commands import is mocked via vi.mock().
  </action>
  ...
  <acceptance_criteria>
    - `npm test` exits 0 with chatStore.test.ts and sessionStore.test.ts results listed
    - `grep "appendStreamingContent|finalizeStream|clearSession" tauri-app/src/stores/chatStore.test.ts` returns 3 matches
    - `grep "loadSessions|UUID validation" tauri-app/src/stores/sessionStore.test.ts` returns 2 matches
    - `grep "vi.mock.*bindings" tauri-app/src/stores/sessionStore.test.ts` confirms Tauri commands are mocked
  </acceptance_criteria>

  <done>Store unit tests created and passing. chatStore: appendStreamingContent accumulation, finalizeStream commit, clearSession reset. sessionStore: empty list, UUID validation comment. All tests green.</done>
</task>
