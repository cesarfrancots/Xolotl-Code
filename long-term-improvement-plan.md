...
- [x] **T-5.1.1 — `xolotl agent --protocol ndjson`.** Stable NDJSON event stream over stdio reusing runtime events (TextDelta/ReasoningDelta/ToolUse/ToolResult). Document schema in `docs/headless-protocol.md`. Test first: golden event-stream for a scripted (mocked) session. Verify: `cargo test`.
  - **SHIPPED (2026-05-30):** `headless.rs` (`HeadlessEvent` serde-tagged enum + `events_from_turn`/`to_ndjson`); turn emitted post-hoc from `TurnSummary` (deterministic, golden-tested) — each `tool_result` interleaved after its `tool_use` by id, stream ends `usage`→`turn_complete`, `error` on failure (exit 1). stdout kept pure via a process-wide `STREAM_OUTPUT` gate on: the 5 inline assistant-text write sites (main.rs ×2, bedrock.rs ×2, openai.rs ×1) **and the `CliToolExecutor::execute` tool-display box** (covers `task`/sub-agent executors too); the `ask_user` tool refuses under `XOLOTL_HEADLESS=1` (no stdout/stdin). Headless turn skips memory injection + cost footer. Prompt from args or piped stdin; interactive-TTY-with-no-prompt and stdin-read failures emit a terminal `error` event (never a hang/bare stderr). `agent` subcommand + `--protocol ndjson` wired through the hand-rolled `parse_args`. Docs in `docs/headless-protocol.md` (honest trust-boundary note — sandbox is opt-in/off, ask_user disabled). DoD green: fmt + CI-exact clippy + tests (CLI golden ×3, tools ask_user regression ×1). Decision: turn-granularity events (not per-delta) — matches the golden-test contract; per-delta streaming deferred.
  - **Adversarial review (12-agent workflow, 4 dims + refute-verify): 8 raw → 7 confirmed, ALL fixed before push.** CRITICAL: tool-display box leaked to stdout (the headline catch — golden tests can't see it). HIGH: `ask_user` stdout-write + stdin-block. MEDIUM ×2: sub-agent/`task` boxes (same gate), false sandbox doc claim. LOW ×3: TTY hang, stdin-fail→stderr-not-NDJSON, wrong docstring. 1 refuted (bare `agent ndjson` positional — undocumented form, out of scope). LESSON holds: phase-boundary adversarial review remains HIGH value. Live tool-call NDJSON-purity smoke test needs API keys → deferred (offline grep confirms `execute` is the sole tool-path stdout chokepoint and it's gated).
