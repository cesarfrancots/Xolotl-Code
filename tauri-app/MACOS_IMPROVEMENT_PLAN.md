...
- Workbench > Latest Agent Worktree and the optional menu bar status item expose Finder, external editor, external terminal, POSIX path, `xolotl-code://` link, and shell open-command handoffs for the most relevant agent run.
...
- Expanded agent output keeps primary worktree Finder/editor/terminal actions visible while grouping worktree copy automation in a compact Mac-style menu; the same latest-agent worktree handoffs are also available from the native Workbench menu and optional menu bar status item.
