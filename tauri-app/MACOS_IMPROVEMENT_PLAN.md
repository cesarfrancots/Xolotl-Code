...
- Workbench > Latest Agent Worktree and the optional menu bar status item expose Finder, external editor, embedded terminal, external terminal, POSIX path, `xolotl-code://` link, and shell open-command handoffs for the most relevant agent run.
...
- The optional menu bar status item can open the Eval workspace directly when eval work exists.
...
- Add task-result handoffs:
  - Reveal artifacts, copy paths, copy deep links, copy shell-open commands, and open folders in editor, embedded terminal, or external terminal from eval/agent result surfaces. Done for launched eval artifact folders and expanded agent worktree folders, including `xolotl-code://` link-copying and Terminal-safe shell-open command handoffs; latest-agent worktree embedded-terminal launch is available from the native Workbench menu and optional menu bar status item.
...
- Improve the optional menu bar status item:
  ...
  - Add quick commands only when they are stable and already route through native command actions. Done for latest-agent output, latest-agent worktree handoffs, and active-eval workspace handoffs; latest-agent output and worktree handoff routing are also available from the native Workbench menu.
...
3. Add Phase 2.5 keyboard parity:
   - Additional file-browser row commands in the command palette. Done for current-folder, recent-folder, Quick Look, New Terminal Here, external-editor handoff, and visible-entry commands.
